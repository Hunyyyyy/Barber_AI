...
// Vì là Server Component, ta có thể gọi hàm async trực tiếp
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative bg-black text-white overflow-hidden">
        <div className="absolute inset-0 opacity-20 bg-[url('https://images.unsplash.com/photo-1585747860715-2ba37e788b70?q=80&w=2074&auto=format&fit=crop')] bg-cover bg-center" />
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 space-y-20">
          ...
          <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
            ...
            <div className="space-y-1 text-gray-600">
              <p className="flex justify-between">
                <span>Sáng:</span> <span className="font-semibold text-black">{settings.morningOpen} - {settings.morningClose}</span>
              </p>
              <p className="flex justify-between">
                <span>Chiều:</span> <span className="font-semibold text-black">{settings.afternoonOpen} - {settings.afternoonClose}</span>
              </p>
            </div>
          </div>
          ...
          <div className="bg-black text-white p-8 rounded-3xl shadow-xl flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="font-bold tracking-widest text-sm text-gray-400 uppercase">Top Stylist</span>
              </div>
            ...
            <div className="flex -space-x-4 mt-6">
              {barbers.slice(0, 4).map((barber: any, idx: number) => (
                <div key={barber.id || idx} className="w-12 h-12 rounded-full border-2 border-black bg-gray-800 flex items-center justify-center font-bold text-xs" title={barber.name}>
                  {barber.name[0]}
                </div>
              ))}
        ...
        <section id="services">
          <div className="text-center mb-12">
            <h2 className="text-3xl lg:text-4xl font-black mb-4">Bảng Giá Dịch Vụ</h2>
            ...
            {services.map((service: any) => (
              <div key={service.id} className="group bg-white p-6 rounded-2xl border border-gray-100 hover:border-black hover:shadow-xl transition-all duration-300">
                <div className="flex justify-between items-start mb-4">
                  <div className="w-12 h-12 bg-gray-50 group-hover:bg-black group-hover:text-white rounded-xl flex items-center justify-center transition-colors">
                    <Scissors className="w-6 h-6" />
                  </div>
                  <span className="font-black text-xl">{formatCurrency(service.price)}</span>
                </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-3xl font-black">Barber Team</h2>
            <Link href="/queue" className="hidden sm:flex items-center gap-2 font-bold hover:underline">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {barbers.map((barber: any) => (
              <div key={barber.id} className="bg-white p-4 rounded-2xl text-center border border-gray-100 hover:shadow-lg transition-all">
                <div className="w-24 h-24 mx-auto bg-gray-100 rounded-full mb-4 flex items-center justify-center text-3xl font-black text-gray-300">
                  {barber.name[0]}
                </div>
                <h3 className="font-bold text-lg">{barber.name}</h3>
                <div className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold mt-2 ${barber.isBusy ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
